  ...
  <div class="modal-overlay">
    <div class="modal new-question-modal">
      ...
      <section class="new-question-content">
        ...
        <select class="type-select" :default="2" @change="newQwiz.idCategory = Number(($event.target as HTMLSelectElement).value)" @input="errorMessage =''">
              <option value="">Ajouter une catégorie</option>
              <option  :value="category.idCategory" v-for="category in categories"
            :key="category.idCategory" :selected="newQwiz.idCategory === category.idCategory">{{ category.name }}</option>
            </select>
...
const newQwiz = ref({ 
  name: currentQuiz.value.name, 
  idCategory: currentQuiz.value.idCategory,
});
...
const submitForm = async () => {
  ...
  if (newQwiz.value.name === "" || !newQwiz.value.idCategory) {
    ...
    return;
  } else if (quizsName.value.includes(newQwiz.value.name) && newQwiz.value.name!== currentQuiz.value.name  ) {
    errorMessage.value = "Vous avez déja un qwiz avec ce nom";
    return;
  } else {
    try {
      const response = await axios.put(`/quiz/${currentQuiz.value.idQuiz}`, newQwiz.value);
